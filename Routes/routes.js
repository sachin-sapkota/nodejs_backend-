...
router.post("/fullquote", async (req, res) => {
  try {
    const access_token = await GetAccessToken();
    const response = await axiosInstance.post(
...
router.post("/issuequote", async (req, res) => {
  try {
    const access_token = await GetAccessToken();
...
router.post("/blockquote", async (req, res) => {
  try {
    const access_token = await GetAccessToken();
    const response = await axiosInstance.post(
...
router.get("/fetchalldocs/:entity/:value", async (req, res) => {
  try {
...
router.get("/downloaddoc/:entity/:value", async (req, res) => {
  try {
    const { entity, value } = req.params;
    const access_token = await GetAccessToken();
    const response = await axiosInstance.get(
...
router.get("/Policies/:ProposalNo", async (req, res) => {
  try {
